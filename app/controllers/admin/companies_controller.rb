  ...
  private
    ...
    def create_csv
      @csv_file = CSV.generate({}) do |csv|
        csv << ['Comany Name', 'Contact Name', 'Email', 'State/Province', 'Country', 'Date Created', 'Disabled?']
        @companies.each do |c|
          csv << [c.name, c.contact_name, c.email, c.state, c.country.try(:upcase), c.created_at, c.disabled]
        end
      end
    end
