class Company::ReviewsController < Company::BaseController
  ...
  def create
    ...
    if @review.save
      Notification.create(title: @job.title, body: "#{@job.company.name} reviewed your performance on this job.", authorable: @job.company, receivable: @job.freelancer, url: freelancer_job_review_url(@job))
