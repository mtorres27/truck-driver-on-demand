  ...
  def create
    ...
    if @review.save
      @job.collaborators_for_notifications.each do |collaborator|
        CompanyMailer.notice_freelancer_review(collaborator, current_user, @review).deliver_later
        Notification.create(title: @job.title, body: "#{current_user.first_name_and_initial} reviewed your performance on this job", authorable: @job.freelancer, receivable: collaborator, url: company_job_review_url(@job))
      end
